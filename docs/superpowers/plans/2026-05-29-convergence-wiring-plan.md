...
- **Next slice:** F5 assessment (is R5 Top-N feasible?) → then QC. F1–F4 done (259 green); live fan-out wired end-to-end.
...
- [x] **F2** — `normalizeCandidate(result, source)` + `extractRepoIdentity(result)`: uniform `{nameWithOwner, sources:[source], hint}` shape (extract the GitHub repo a web/academic result references; null if none) → feeds `phase2Convergence`. **8 tests incl. a phase2Convergence integration proving exa+github = 2 families; 249 green.** Reserved-owner guard (sponsors/topics/gist) keeps family-count honest (R9).
